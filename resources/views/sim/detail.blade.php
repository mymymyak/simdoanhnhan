...
<ul class="sim-info">
    ...
    <div class="col-right">
        <img src="/{{$sim['SIMURL']}}.jpg"/>
    </div>
</div>
<?php if (!empty($this->suggest)) : ?>
            ...
            foreach ($this->suggest as $item) {
                ...
                <div class="divTableRow">
                    <div class="divTableCell cell-left">
                        <a class="green bold sosim" href="/<?= $sim_url ?>"><?= $line->f ?></a>
                    </div>
                    <div class="divTableCell cell-right cell-price"><b><?= $gia ?>₫</b></div>
                    <div class="divTableCell hidden-xs">
                        <div class="mang">
                            <img src="<?= '/public/images2/' . $mangimg . '.png' ?>" alt="<?= $mang ?>" />
                        </div>
                    </div>
...
<div class="form-order" id="form-order">
    ...
    <form name="order_form" id="order_form" style="padding-top:30px;padding-bottom:30px">
        ...
        <div class="form-groups">
            ...
            <div class="form-input">
                <input type="text" name="order_phone" id="dienthoai" placeholder="Nhập số điện của bạn" />
            </div>
        </div>
		<div class="form-groups">
        ...
        <?php $clshot = isMobile() ? ' class="hotdetail"' : ''; ?>
		<?php
		$vip = '';
        ...
        <div class="form-groups group-submit">
			...
			<p class="txt-help">Đặt sim trực tiếp qua Hotline: <a<?= $clshot ?> href="tel:09.6888.7888">09.6888.7888</a></p>
        </div>
    </form>
</div>
<div class="hd">
...
<ul><li>Cách 1: CHONSIMVIETTEL sẽ giao sim trong ngày và thu tiền tại nhà <i>(áp dụng tại các thành phố, thị trấn lớn)</i></li><li>Cách 2: Quý khách đến cửa hàng TOP SIM để nhận sim trực tiếp <i>(Danh sách của hàng ở chân website)</i></li><li>Cách 3: CHONSIMVIETTEL sẽ giao sim theo đường bưu điện và thu tiền tại nhà. </li></ul>
...
<?php endif; ?>
<script>var class2tr = '<?php echo $sim['GIABAN2'] <= 2000000 ? 'hotlime_desktop' : 'hotline_mobile'; ?>'</script>
@stop
